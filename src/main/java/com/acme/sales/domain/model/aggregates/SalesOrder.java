...
public class SalesOrder {
    ...
    public void addItem(int quantity, double unitPrice, Long productId) {
        if (this.status == SalesOrderStatus.APPROVED) throw new IllegalStateException("Cannot add items to an approved order");
    ...
    public void addPayment(double amount) {
        if (amount <= 0) throw new IllegalArgumentException("Payment amount must be greater than 0");
        if (amount > calculateTotalPrice() - paymentAmount) throw new IllegalArgumentException("Payment amount exceeds the total price");
        this.paymentAmount += amount;
        verifyIfReadyToDispatch();
    }

    private void verifyIfReadyToDispatch() {
